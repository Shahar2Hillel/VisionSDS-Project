    ...
    try:
        ...
        for i, obj_path in enumerate(obj_paths):
            ...
            if len(obj_paths) > 1:
                ...
                offset_mag = random.uniform(0.7, 1.0)
        ...
        if len(loaded_objects) > 1:
            ...
            jitter = random.uniform(0.03, 0.07) * 3.0 # small world jitter
        ...
        while tries < max_tries and poses < num_images:
            ...
            if scene_type == "single":
                ...
                location = bproc.sampler.shell(
                center=scene_center, radius_min=9.0, radius_max=12.0, elevation_min=75, elevation_max=85
                )
                lookat_point = scene_center
                rotation_matrix = bproc.camera.rotation_from_forward_vec(
                lookat_point - location, inplane_rot=np.deg2rad(np.random.uniform(-98, -92))
                )
